 ...
 */
export function extractSubsidyInfo(responseText: string): SubsidyInfo[] {
  const subsidyInfoList: SubsidyInfo[] = [];
 ...
 */
export function extractSubsidyIds(responseText: string): string[] {
 ...
 */
export function getFirstSubsidyId(responseText: string): string | null {
  const subsidyInfo = extractSubsidyInfo(responseText);
 ...
 * 補助金一覧の応答を解析する
 */
export interface ParsedSubsidyResponse {
  subsidies: SubsidyInfo[];
  hasMultiple: boolean;
}

export function parseSubsidyListResponse(agentResponse: string): ParsedSubsidyResponse {

...
def summarize_url(url, llm):
    print(f"\n--- {url} ---")
    query = f"次のURLの補助金の予算額を一単語だけで出力してください。出力は必ず1語のみ（例: 200万円, 500円 など）説明文や補足は禁止。：{url}"
    resp = llm.invoke(
        query,
...
def main():
    ...
    # URL列に対してsummarize_urlを適用して「amount」列を作成
    formatted_df['amount'] = formatted_df['url'].map(lambda url: summarize_url(url, llm))
